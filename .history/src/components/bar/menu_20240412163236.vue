  ...
  <div v-if="needNav && !whiteList.includes(route.path)">
    <div class="nav-container" :class="{ theme }" :style="getHeaderStyle">
      <div v-if="isSwitchLanguages" class="switch-language-dialog">
        ...
        <div class="switch-language-container">
          ...
          <div class="language-list-container">
            <div
              v-for="(la, lIndex) in languageList"
              class="language-item"
              :class="{
                'last-item': (lIndex + 1) % 3 === 0,
                langSelected: la.isSelected,
              }"
      ...
      <div
        ...
        :style="{
          'background-color':
            theme === 'nav-top' ? 'transparent' : 'rgba(255,255,255)',
        }"
      >
        <nav>
          <div class="left-space">
            ...
            <div
              class="menu-space"
              :style="{ width: menuList.length * 90 + 'px' }"
            >
              <div
              ...
              >
                ...
                <div
                  v-if="menu.subMeuns.length > 0"
                  class="sub-menu"
                  @mouseleave="menu.subMenuVisible = false"
                  @mouseover="menu.subMenuVisible = true"
                  :style="{
                    height: menu.subMenuVisible
                      ? menu.subMeuns.length * 44 + 'px'
                      : 0 + 'px',
                  }"
                >
                  <div
                    v-for="(sub, subIndex) in menu.subMeuns"
                    class="sub-menu-item"
                    @click="gotoTargetPageBySub(sub)"
                    :style="{
                      opacity: menu.subMenuVisible ? 1 : 0,
                      display: menu.subMenuVisible ? '' : 'none',
                    }"
                  >
                    <p>{{ t(sub.name) }}</p>
                  </div>
                ...
                <div
                  v-if="isMenuUnderlined(index)"
                  class="underline"
                  :style="
                    theme === 'nav-top' ? 'background-color: rgba(0,0,0)' : ''
                  "
                ></div>
              </div>
            </div>
                ...
                >
                  ...
                  <span class="btn-search" @click="search">
                    <img
                      ...
                      :src="
                        theme === 'nav-top'
                          ? 'https://tentech.oss-cn-shenzhen.aliyuncs.com/icon/search-icon1.svg'
                          : 'https://tentech.oss-cn-shenzhen.aliyuncs.com/icon/search-icon.svg'
                      "
                    />
                  </span>
                </span>
              ...
              <li v-if="showUser" class="user-bar">
                <router-link to="/login" class="nav-btn">
                  <img
                    ...
                    :src="
                      theme === 'nav-top'
                        ? 'https://tentech.oss-cn-shenzhen.aliyuncs.com/icon/user-icon1.svg'
                        : 'https://tentech.oss-cn-shenzhen.aliyuncs.com/icon/user-icon.svg'
                    "
                  />
                </router-link>
              </li>
    ...
    <div class="drawer-nav">
      <div
        class="nav-content"
        :style="{
          'background-color': theme === 'nav-top' ? 'transparent' : '#fff',
        }"
      >
        ...
        <div class="drawer-space">
          <div
            ...
            :style="{
              'background-color':
                theme === 'nav-top' ? 'transparent' : '#f3f3f3',
            }"
          ...
          >
            ...
            >
              {{ LanguageComparison.get(currentLanguage) }}
            </p>
          </div>
        </div>
      </div>
      <div
        ...
        :style="
          isExpandMenu
            ? 'height:100%;width:100%;opacity: 1;overflow: auto;'
            : 'height:0px;width:0px;opacity: 0;'
        "
      >
        <div class="drawer-menu-space">
          <div
            v-for="(drawerMenu, drawerMenuIndex) in menuList"
            class="drawer-menu-item"
            :style="{
              height: drawerMenu.isExpand
                ? drawerMenu.subMeuns.length * 40 + 70 + 'px'
                : '50px',
            }"
          ...
          >
            <div class="menu-content" @click="goToExpandSubmenu(drawerMenu)">
              <div class="menu-button">
                <h1
                  :style="{ 'font-weight': drawerMenu.isExpand ? 'bold' : '' }"
                >
                  {{ t(drawerMenu.name) }}
                </h1>
                <img
                  ...
                  "
                  src="../../assets/images/common/right-arrow-black.svg"
                />
              </div>
            </div>
            <div
              class="sub-menu-space"
              :style="{
                height: drawerMenu.isExpand
                  ? drawerMenu.subMeuns.length * 40 + 'px'
                  : '0px',
                opacity: drawerMenu.isExpand ? '1' : '0.8',
              }"
            >
              <div
                :style="{
                  opacity: drawerMenu.isExpand ? '1' : '0',
                  display: drawerMenu.isExpand ? '' : 'none',
                  height: drawerMenu.isExpand ? '40px' : '0px',
                }"
                v-for="(sub, subIndex) in drawerMenu.subMeuns"
                class="drawer-sub-menu-item"
                @click="gotoTargetPageBySub(sub)"
              >
                <p>{{ t(sub.name) }}</p>
              </div>
...
const menuList = ref([
    ...
    subMeuns: [
      {
        ...
        subMeuns: [
          ...
          {
            name: "menu.fixedInstallLEDDisplay",
            path: "/products",
            seriesId: "fixed-install-led-display",
          },
          {
            name: "menu.fineSmallPixelLEDDisplay",
            path: "/products",
            seriesId: "fines-mall-pixel-led-display",
          },
          {
            name: "menu.transparentLEDDisplay",
            path: "/products",
            seriesId: "transparent-led-display",
          },
          {
            name: "menu.iposterLEDDisplay",
            path: "/products",
            seriesId: "iposter",
          },
          {
            name: "menu.creativeDisplay",
            path: "/products",
            seriesId: "creative-display",
          },
          { name: "menu.ledModule", path: "/products", seriesId: "led-module" },
          {
            name: "menu.ledDisplayAccessories",
            path: "/products",
            seriesId: "led-display-accessories",
          },
        ],
      },
      {
  ...
  },
  {
    ...
    subMeuns: [
      {
        name: "menu.companyIntroduction",
        path: "/aboutus",
        seriesId: "companyIntro",
      },
      {
        name: "menu.brandQualifications",
        path: "/aboutus",
        seriesId: "certificate",
      },
      {
        name: "menu.teamServices",
        path: "/aboutus",
        seriesId: "24hours-service",
      },
...
const getHeaderStyle = computed(() => {
  if (relativeList.value.includes(route.path)) {
    //这里是不跟随路径
    return {
      position: "relative",
    };
