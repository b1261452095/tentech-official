    ...
    <div class="banner">
      ...
      <div class="select-button-container">
        <div class="button-container">
          <div
            class="about-button"
            :class="{ buttonSelected: ab.isSelected }"
            v-for="(ab, abIndex) in selectMenu"
    ...
    <adaptive-container>
      ...
      <div v-if="currentDisplay === 2" class="data-download">
        <div class="search-bar">
          <search-bar
            :btn="t('menu.searchFind')"
            v-model="inputQuestion"
            :placeholder="t('menu.inputSearchFile')"
            maxlength="64"
            @search="search"
          ></search-bar>
        </div>
        <div class="download-content">
          <div class="sidebar">
            <div
              ...
              class="sidebar-menu"
              :class="{ 'expand-menu': item.isExpand }"
              :style="{
                height: item.isExpand
                  ? item.subMenu.length * 48 + 48 + 'px'
                  : '48px',
              }"
            >
              <div class="sidebar-menu-button" @click="goToExpand(item, index)">
                ...
                <img
                  ...
                  src="https://tentech.oss-cn-shenzhen.aliyuncs.com/svg/arrow-right.svg"
                />
              </div>
              <div
                class="sub-menu-list"
              ...
              >
                ...
                >
                  <h1
                    :style="
                      sub.isSelected ? 'color:#000;font-weight: 700;' : ''
                    "
                  >
                    {{ t(sub.name) }}
                  </h1>
          ...
          <div class="help-list-container">
            ...
            >
              ...
              <div class="help-desc-container">
                <p>{{ t("发布时间") }}:{{ item.date }}</p>
                <p>{{ t("点赞数量") }}:{{ item.likeNumber }}</p>
              </div>
            </a>
          </div>
        </div>
      ...
      <div v-if="currentDisplay === 3" class="data-download">
        <div class="search-bar">
          <search-bar
            :btn="t('menu.searchFind')"
            v-model="inputQuestionFaq"
            :placeholder="t('menu.PleaseInput')"
            maxlength="64"
            @search="search"
          ></search-bar>
        </div>
        ...
        <div class="download-content">
          <div class="sidebar">
            <div
              v-for="(item, index) in faqMenuList"
              class="sidebar-menu"
              :class="{ 'expand-menu': item.isExpand }"
              :style="{
                height: item.isExpand
            ...
            >
              <div
                ...
                @click="goToFaqExpand(item, index)"
              >
                <h1>{{ item.title }}</h1>
                <img
                  v-if="item.subMenu.length > 0"
...
const filteredFaqList = computed(() => {
  return currentFaqList.value.filter((faq) => {
    ...
    const contentContainsKeyword = faq.faqQuestion
      .toLowerCase()
      .includes(keyword);
    return titleContainsKeyword || contentContainsKeyword;
  });
...
watch(route, (newValue) => {
  if (route.path === "/service" && newValue.query.seriesId) {
    ...
    selectMenu.value.find(
      (se) => se.menuName === newValue.query.seriesId
    ).isSelected = true;
    currentDisplay.value = selectMenu.value.find(
      (se) => se.menuName === newValue.query.seriesId
    ).menuValue;
    servicePageInfo.updateCurrentDisplay(currentDisplay.value);
...
onBeforeMount(() => {
  if (route.path === "/service" && route.query.seriesId) {
    ...
    selectMenu.value.find(
      (se) => se.menuName === route.query.seriesId
    ).isSelected = true;
    currentDisplay.value = selectMenu.value.find(
      (se) => se.menuName === route.query.seriesId
    ).menuValue;
    ...
    selectMenu.value.find(
      (se) => se.menuValue === servicePageInfo.currentDisplay
    ).isSelected = true;
    currentDisplay.value = selectMenu.value.find(
      (se) => se.menuValue === servicePageInfo.currentDisplay
    ).menuValue;
